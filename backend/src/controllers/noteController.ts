...
export const getNoteById = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  try {
    if (global.isMockDatabase) {
      ...
      const note = await Note.findById(id).populate('uploadedBy', 'name email avatar');
      if (!note) {
        res.status(404).json({ message: 'Note not found' });
        return;
      }
      ...
      // Populate reviews
      const populatedReviews = await Review.find({ noteId: id }).populate('userId', 'name avatar');
...
export const uploadNote = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { title, description, subject, tags, fileName, fileUrl } = req.body;
