...
export const AppProvider = ({ children }) => {
   ...
   * Supports optional filters query parameters.
   */
  const fetchNotes = async (user = currentUser, filters = {}) => {
    try {
      ...
      const res = await fetch(`${API_BASE}/notes?${queryString}`, {
        headers: getHeaders()
      });
      let fetchedNotes = [];
      if (res.ok) {
  ...
  /**
   * Fetches the complete list of Past Year Questions with optional query filters.
   */
  const fetchPYQs = async (filters = {}) => {
    try {
      const queryParams = new URLSearchParams();
      Object.entries(filters).forEach(([key, val]) => {
        if (val !== undefined && val !== null && val !== '' && val !== 'All') {
      ...
      const queryString = queryParams.toString();
      const url = queryString ? `${API_BASE}/pyqs?${queryString}` : `${API_BASE}/pyqs`;
  ...
  const updateProfile = async (name, avatar, rollNumber, branch, semester, campusBlock) => {
    const res = await fetch(`${API_BASE}/auth/profile`, {
      method: 'PUT',
      headers: getHeaders(),
  ...
  const uploadNote = async (title, description, subject, tags, fileName, fileUrl, semester, syllabusCode, campusBlock) => {
  ...
  const uploadPYQ = async (year, subject, university, difficultyLevel, fileName, fileUrl, semester, syllabusCode, campusBlock) => {
    const res = await fetch(`${API_BASE}/pyqs`, {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify({ year, subject, university, difficultyLevel, fileName, fileUrl, semester, syllabusCode, campusBlock })
    });

    const json = await res.json();
    if (!res.ok) throw new Error(json.message || 'Failed to upload PYQ');
  ...
  const deletePYQ = async (pyqId) => {
    ...
    const json = await res.json();
    if (!res.ok) throw new Error(json.message || 'Failed to delete PYQ');
  ...
  const trackPYQDownload = async (pyqId) => {
    try {
      ...
      console.error('Failed to track PYQ download:', err);
    }
  };
