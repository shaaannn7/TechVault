...
export default function Dashboard({ setActivePage, setViewingDoc }) {
  ...
  const handleProfileSave = (e) => {
    e.preventDefault();
    if (newName.trim()) {
      updateProfile(newName.trim(), newAvatar);
      setEditMode(false);
    }
  };
      ...
      <div className="glass-card rounded-2xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left w-full">
          ...
          <div className="flex flex-col gap-1 w-full sm:w-auto">
            {editMode ? (
              <form onSubmit={handleProfileSave} className="flex flex-col sm:flex-row items-center gap-2 mt-1">
                <input
                  type="text"
                  value={newName}
                  onChange={(e) => setNewName(e.target.value)}
                  className="px-3 py-1.5 rounded-lg bg-slate-950/60 border border-white/5 focus:border-violet-500 text-sm outline-none text-slate-200"
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => { setEditMode(false); setNewName(currentUser.name); setNewAvatar(currentUser.avatar); }}
