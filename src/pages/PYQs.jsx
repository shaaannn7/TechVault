...
export default function PYQs({ viewingPYQ, setViewingPYQ }) {
  ...
  // JECRC / RTU Specific upload fields
  const [upSemester, setUpSemester] = useState(1);
  const [upSyllabusCode, setUpSyllabusCode] = useState('');
  const [upCampusBlock, setUpCampusBlock] = useState('A-Block (CSE/IT Department)');
  ...
  const filteredPYQs = useMemo(() => {
    ...
    return pyqs.filter(paper => {
      const matchesSearch = !searchLower ||
                            ...
                            (paper.syllabusCode && paper.syllabusCode.toLowerCase().includes(searchLower));
      ...
      const matchesSemester = semesterFilter === 'All' || (paper.semester && paper.semester.toString() === semesterFilter);

      return matchesSearch && matchesUniversity && matchesYear && matchesDifficulty && matchesSemester;
    });
  }, [pyqs, search, universityFilter, yearFilter, difficultyFilter, semesterFilter]);
  ...
  const handleUploadSubmit = async (e) => {
    ...
    try {
      ...
      await uploadPYQ(upYear, upSubject, upUniversity, upDifficulty, uploadData.fileName, uploadData.fileUrl, upSemester, upSyllabusCode, upCampusBlock);
      ...
      setTimeout(() => {
        ...
        setUpSemester(1);
        setUpSyllabusCode('');
        setUpCampusBlock('A-Block (CSE/IT Department)');
        setUploadOpen(false);
        setFormSuccess('');
      }, 1500);
    } catch (err) {
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 flex flex-col gap-6 text-slate-200">
      ...
      {!viewingPYQ ? (
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center bg-slate-900/10 border border-white/5 p-4 rounded-2xl">
            {/* Search Input */}
            <div className="relative md:col-span-1">
              ...
              <input
                type="text"
                placeholder="Search subject or syllabus code..."
                value={search}
            ...
            {/* Semester Filter */}
            <div className="flex flex-col gap-1 text-left">
              <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest pl-1">Semester</label>
              <select
                value={semesterFilter}
                onChange={(e) => setSemesterFilter(e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl bg-slate-950/40 border border-white/5 text-xs text-slate-300 outline-none cursor-pointer focus:border-violet-500"
              >
                <option value="All" className="bg-[#0b0f19]">All Semesters</option>
                {[1, 2, 3, 4, 5, 6, 7, 8].map(s => (
                  <option key={s} value={s.toString()} className="bg-[#0b0f19]">Sem {s}</option>
                ))}
              </select>
            </div>
              ...
              {filteredPYQs.map(paper => (
                ...
                >
                  <div>
                    ...
                    <div className="flex flex-wrap justify-between items-start gap-2 mb-3">
                      <div className="flex flex-wrap gap-1">
                        <span className="text-[9px] font-black uppercase tracking-wider text-violet-400 bg-violet-500/10 border border-violet-500/20 px-2 py-0.5 rounded-md">
                          {paper.university.split(' ')[0]}
                        </span>
                        {paper.semester && (
                          <span className="text-[9px] font-bold uppercase tracking-wider text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-md">
                            Sem {paper.semester}
                          </span>
                        )}
                        {paper.syllabusCode && (
                          <span className="text-[9px] font-mono font-bold uppercase tracking-wide text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-md">
                            {paper.syllabusCode}
                          </span>
                        )}
                      </div>
                    ...
                    {/* Paper Title */}
                    <h3 className="text-sm font-extrabold text-slate-200 group-hover:text-violet-400 transition-colors mb-0.5">
                      {paper.subject} Exam Paper
                    </h3>
                    <div className="flex items-center gap-x-2 text-[10px] text-slate-500 font-semibold mb-3">
                      <span>Year: {paper.year}</span>
                      {paper.campusBlock && (
                        <>
                          <span>•</span>
                          <span>{paper.campusBlock.split(' ')[0]}</span>
                        </>
                      )}
                    </div>
                  </div>
              ...
              <div className="glass-card rounded-2xl p-6 border border-white/5 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2 text-xs border-t border-white/5 pt-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-slate-300 font-bold">{viewingPYQ.uploadedBy?.name || 'Academic Contributor'}</span>
                  </div>
                  {viewingPYQ.semester && (
                    <div className="flex justify-between">
                      <span className="text-slate-500 font-medium">RTU Semester</span>
                      <span className="text-indigo-400 font-bold">Semester {viewingPYQ.semester}</span>
                    </div>
                  )}
                  {viewingPYQ.syllabusCode && (
                    <div className="flex justify-between">
                      <span className="text-slate-500 font-medium">RTU Syllabus Code</span>
                      <span className="text-emerald-400 font-mono font-bold">{viewingPYQ.syllabusCode}</span>
                    </div>
                  )}
                  {viewingPYQ.campusBlock && (
                    <div className="flex justify-between">
                      <span className="text-slate-500 font-medium">Classroom Block Location</span>
                      <span className="text-slate-300 font-semibold">{viewingPYQ.campusBlock}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    value={upSyllabusCode}
                    onChange={(e) => setUpSyllabusCode(e.target.value)}
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select
                    value={upCampusBlock}
                    onChange={(e) => setUpCampusBlock(e.target.value)}
                    className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950/40 border border-white/5 focus:border-violet-500 text-xs outline-none text-slate-200 cursor-pointer"
                  >
